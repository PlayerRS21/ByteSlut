...
# IMPORTANT: We must back up settings from the EXISTING INSTALL, not from the
# new archive. The new archive has default settings. The existing install has
# YOUR customized settings (app name, CLI command, port, daily report time, etc.)
#
# Step order: determine existing install path FIRST, then back up from there.
SERVICE_FILE="$HOME/.config/systemd/user/byteslut.service"
EXISTING_SETTINGS=""

# Try to find existing install via systemd service
...
# Fallback: if NEW_ROOT itself has a settings.json that looks like it was
# customized (cli_command != "byteslut" or app_name != "ByteSlut"), use it.
# This handles the case where the user runs update.sh in-place.
if [ -z "$EXISTING_SETTINGS" ] && [ -f "$NEW_ROOT/config/settings.json" ]; then
    EXISTING_SETTINGS="$NEW_ROOT/config/settings.json"
    info "Using settings from current directory: $EXISTING_SETTINGS"
fi
