...
def run_data_retention():
    """
    Auto-aggregate and purge old data to keep DB size manageable.
    
    RETENTION POLICY (researched from SQLite time-series best practices):
    ┌─────────────────────┬──────────────────┬─────────────────────────────┐
    │ Table               │ Raw retention    │ What we keep forever        │
    ├─────────────────────┼──────────────────┼─────────────────────────────┤
    │ system_stats        │ 7 days           │ Hourly averages (cpu,ram,temp)│
    │ input_stats         │ 30 days          │ Daily totals                │
    │ app_usage           │ 90 days          │ Daily totals per app        │
    │ browser_history     │ 180 days         │ Domain daily summaries      │
    │ sessions, commands  │ Forever (small)  │ All                         │
    │ notifications       │ 90 days          │ Daily counts by app         │
    │ network_usage       │ 90 days          │ Daily totals                │
    └─────────────────────┴──────────────────┴─────────────────────────────┘
    
    HOW IT WORKS:
    1. For each table with a retention window:
       a. Aggregate old rows into daily/hourly summary rows
       b. Delete the original high-frequency rows
    2. Run PRAGMA optimize (SQLite 3.46+) or analyze 
    3. This reduces 5MB/day to ~200KB/day long-term:
       - system_stats alone drops from ~600KB/day to ~5KB/day (hourly avgs)
       - input_stats drops from ~300KB/day to ~1KB/day (daily totals)
    """
    ...
    try:
        ...
        # ── system_stats: aggregate raw → hourly beyond 7 days ──────────
        # Keep last 7 days raw for the hardware page graphs.
        # Beyond that: one row per hour with averaged values.
        c.execute("""
            INSERT OR IGNORE INTO system_stats_hourly
              (hour_ts, cpu_avg, ram_avg, swap_avg, temp_avg, load_avg, sample_count)
            SELECT
              (timestamp / 3600) * 3600 as hour_ts,
              ROUND(AVG(cpu_percent), 1),
              ROUND(AVG(ram_percent), 1),
              ROUND(AVG(COALESCE(swap_percent, 0)), 1),
              ROUND(AVG(COALESCE(cpu_temp, 0)), 1),
              0.0,
              COUNT(*)
            FROM system_stats
        ...
        c.execute("""
            ...
            WHERE timestamp < strftime('%s','now','-7 days')
        """)
        logger.info("system_stats: old raw rows aggregated to hourly")
        
        # ── input_stats: keep 30 days raw, daily summaries forever ──────
        c.execute("""
            INSERT OR IGNORE INTO input_stats_daily
              (date, total_keystrokes, total_clicks, total_scroll,
               total_distance_px, avg_wpm, sample_count)
            SELECT
              date,
              SUM(keystrokes),
              SUM(mouse_clicks),
              SUM(mouse_scroll_events),
              SUM(mouse_distance_px),
              ROUND(AVG(CASE WHEN wpm_sample > 0 THEN wpm_sample END), 1),
              COUNT(*)
            FROM input_stats
            WHERE date < date('now','-30 days')
            GROUP BY date
        """)
        c.execute("DELETE FROM input_stats WHERE date < date('now','-30 days')")
        logger.info("input_stats: old raw rows aggregated to daily")
        
        # ── browser_history: drop very old entries beyond 180 days ──────
        # We already have daily_roles and productivity summaries.
        # Raw browser URLs beyond 6 months have little analytics value.
        c.execute("""
            DELETE FROM browser_history
            WHERE date < date('now','-180 days')
        """)
        
        # ── notifications: keep 90 days raw, drop older ──────────────────
        c.execute("DELETE FROM notifications WHERE date < date('now','-90 days')")
        
        # ── network_usage: keep 90 days ───────────────────────────────────
        c.execute("DELETE FROM network_usage WHERE date < date('now','-90 days')")
...
def _ensure_aggregate_tables(cursor):
    """
    Create the aggregate tables used by run_data_retention().
    Called from initialize_database().
    """
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS system_stats_hourly (
            hour_ts      INTEGER PRIMARY KEY,  -- Unix timestamp rounded to hour
            cpu_avg      REAL,
            ram_avg      REAL,
            swap_avg     REAL,
            temp_avg     REAL,
    ...
    """)
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS input_stats_daily (
            date              TEXT PRIMARY KEY,
            total_keystrokes  INTEGER DEFAULT 0,
            total_clicks      INTEGER DEFAULT 0,
            total_scroll      INTEGER DEFAULT 0,
            total_distance_px INTEGER DEFAULT 0,
...
def initialize_database():
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS browser_history (
            ...
            UNIQUE(timestamp, url, browser)   -- prevents re-import on daemon restart
        )
    """)
    # Add unique constraint to existing DBs via migration
    try:
        cursor.execute("""
            CREATE UNIQUE INDEX IF NOT EXISTS idx_browser_unique
            ON browser_history(timestamp, url, browser)
        """)
    except Exception:
        pass  # Already exists or can't be added (duplicate data present)
    ...
    for col, typedef in [
        ...
        try:
            cursor.execute(f"ALTER TABLE daily_roles ADD COLUMN {col} {typedef}")
        except Exception:
    ...
    # Clamp existing browser_history visit_duration_seconds to 300s max.
    # Old rows may have been stored with huge values (background tabs open for hours).
    # This runs on every start but is a no-op after the first run since all new
    # rows are stored capped. Safe to run multiple times.
    try:
        affected = conn.execute(
            "UPDATE browser_history SET visit_duration_seconds = 300 "
            "WHERE visit_duration_seconds > 300"
        ).rowcount
        conn.commit()
        if affected > 0:
            logger.info(
                f"Migration: clamped {affected} browser_history rows "
                f"with visit_duration_seconds > 300"
            )
    except Exception as e:
        logger.debug(f"Migration (visit_duration clamp) skipped: {e}")

    # Create aggregate tables used by data retention (safe to call every startup)
    _ensure_aggregate_tables(conn.cursor())
    conn.commit()
        ...
        try:
            ...
            for table, data in batch:
                try:
                    ...
                    # browser_history has UNIQUE(timestamp,url,browser) — use
                    # INSERT OR IGNORE so daemon restarts don't create duplicates
                    verb = "INSERT OR IGNORE" if table == "browser_history" else "INSERT"
                    conn.execute(
