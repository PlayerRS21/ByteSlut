...
class ByteSlutDaemon:
    ...
    def run(self):
        ...
        priv_sentinel = Path(__file__).parent / ".privacy_changed"

        while self.running:
            # ── Privacy change sentinel ──────────────────────────────────────
            # Written by web/app.py when user saves new privacy settings.
            # We reload privacy in each collector that supports it — no restart needed.
            if priv_sentinel.exists():
                try:
                    import json as _j
                    new_priv = _j.loads(priv_sentinel.read_text())
                    priv_sentinel.unlink()
                    logger.info(f"Privacy settings changed: {new_priv}")
                    for collector in self.collectors:
                        cls = collector.__class__.__name__
                        # All collectors using PrivacyMixin have reload_privacy()
                        # InputCollector also has _reload_privacy() as alias
                        if hasattr(collector, "reload_privacy"):
                            collector.reload_privacy()
                            logger.info(f"  {cls}: privacy reloaded")
                        # CommandCollector: stop installing hooks if track_commands=False
                        if cls == "CommandCollector":
                            collector.enabled = new_priv.get("track_commands", True)
                        # BrowserCollector: skip reading if track_browser_history=False
                        if cls == "BrowserCollector":
                            collector.enabled = new_priv.get("track_browser_history", True)
                        # NotificationCollector
                        if cls == "NotificationCollector":
                            collector.enabled = new_priv.get("track_notifications", True)
                except Exception as e:
                    logger.error(f"Privacy sentinel error: {e}")
